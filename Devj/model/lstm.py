...
class DeepAR(NNModel):
    ...
    '''
        Below is the method that needs to be patched when changing NN structure
        :return: inputs_shape (tuple), inputs (Tensor), [loc, scale] (a list of theta pameameters of the target likelihood).
    ...
    def fit(
        ...
        Args:
            ...
            "auto", 0, or 1.
            patience (int): Number of epochs without improvement to stop.
            """
        if not epochs:
            epochs = self.epochs
    ...
    def predict_theta_from_input(self, input_list):
        """
        This function takes an input of size equal to the n_steps specified in 'Input' when building the network
            :param input_list:
